...
for line in css_content.splitlines(True):
    ...
    # JSCSS-DEL-START - JSCSS-DEL-END block supprime
    if "JSCSS-DEL-START" in stripped:
        in_remove_block = True
        continue
    if "JSCSS-DEL-END" in stripped:
        in_remove_block = False
        continue
    if in_remove_block:
        continue

    # JSCSS-ADD-START - JSCSS-ADD-END dans JSCSS
    if "JSCSS-ADD-START" in stripped:
        continue
    if "JSCSS-ADD-END" in stripped:
        continue
